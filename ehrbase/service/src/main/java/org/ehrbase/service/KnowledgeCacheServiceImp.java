...
public class KnowledgeCacheServiceImp implements KnowledgeCacheService {
    ...
    public KnowledgeCacheServiceImp(
            TemplateStorage templateStorage, CacheProvider cacheProvider, Adl2KnowledgeService adl2KnowledgeService) {
    ...
    private WebTemplate retrieveInternalTemplate(String templateId) {
        Optional<TemplateMetaData> meta = templateStorage.readTemplate(templateId);
        if (meta.isEmpty()) {
            throw new IllegalArgumentException("Could not retrieve template for template Id: " + templateId);
        }
        TemplateMetaData templateMetaData = meta.get();
        if (templateMetaData.isAdl2()) {
            return buildAdl2WebTemplate(templateMetaData);
        }
    ...
    private WebTemplate buildAdl2WebTemplate(TemplateMetaData meta) {
        if (meta.getAdl2OptJson() == null || meta.getAdl2OptJson().isBlank()) {
            throw new IllegalArgumentException("ADL2 template has no stored OPT JSON");
        }
        try {
    ...
    @Override
    public String addAdl2OperationalTemplate(String templateId, String adl2Source, String adl2OptJson) {
        validateAdl2Payload(templateId, adl2OptJson);
        boolean canOverwrite = templateStorage.allowTemplateOverwrite();
        if (!canOverwrite && templateStorage.readTemplate(templateId).isPresent()) {
            throw new StateConflictException(
                    "Operational template with this template ID already exists: " + templateId);
        }
        TemplateMetaData stored = templateStorage.storeAdl2Template(templateId, adl2Source, adl2OptJson);
        invalidateCaches(templateId, stored.getInternalId());
        log.info("Registered ADL2 template: {}", templateId);
        ensureCached(stored);
        return templateId;
    }

    @Override
    public Optional<String> retrieveAdl2OperationalTemplate(String templateId) {
        return templateStorage.readTemplate(templateId)
                .filter(TemplateMetaData::isAdl2)
                .map(TemplateMetaData::getAdl2OptJson);
    }

    @Override
    public Optional<String> retrieveAdl2OperationalTemplateAdl(String templateId) {
        return templateStorage.readTemplate(templateId).filter(TemplateMetaData::isAdl2).map(meta -> {
            if (meta.getAdl2Source() != null && !meta.getAdl2Source().isBlank()) {
                return meta.getAdl2Source();
            }
            if (meta.getAdl2OptJson() != null && !meta.getAdl2OptJson().isBlank()) {
                return adl2KnowledgeService.serializeAdl(adl2KnowledgeService.deserializeOptJson(meta.getAdl2OptJson()));
            }
            return null;
        });
    }

    private void validateAdl2Payload(String templateId, String adl2OptJson) {
        if (templateId == null || templateId.isBlank()) {
            throw new InvalidApiParameterException("Template id is required");
        }
        if (adl2OptJson == null || adl2OptJson.isBlank()) {
            throw new InvalidApiParameterException("ADL2 OPT JSON is required");
        }
        adl2KnowledgeService.deserializeOptJson(adl2OptJson);
    }
