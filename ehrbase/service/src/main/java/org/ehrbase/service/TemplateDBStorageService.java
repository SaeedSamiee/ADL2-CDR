...
public class TemplateDBStorageService implements TemplateStorage {
    ...
    @Override
    public TemplateMetaData storeAdl2Template(String templateId, String adl2Source, String adl2OptJson) {
        if (findUuidByTemplateId(templateId).isEmpty()) {
            return templateStoreRepository.storeAdl2(templateId, adl2Source, adl2OptJson);
        }
        boolean templateUsed = compositionRepository.isTemplateUsed(templateId);
        if (templateUsed && !allowTemplateOverwrite) {
            throw new UnprocessableEntityException(
                    "Cannot update template %s since it is used by at least one composition".formatted(templateId));
        }
        return templateStoreRepository.updateAdl2(templateId, adl2Source, adl2OptJson);
    }
